...
begin
  ...
  if actor not in ('MASTER', 'LIVE', 'RUKO', 'WAREHOUSE') or not public.can_access_location(p_location_id) then
    raise exception 'PERMISSION_DENIED';
  end if;
  if p_idempotency_key is null or p_items is null or jsonb_typeof(p_items) <> 'array' or jsonb_array_length(p_items) = 0 then
  ...
  for item in select * from jsonb_array_elements(p_items) loop
    if coalesce((item->>'quantity')::integer, 0) <= 0 or coalesce((item->>'unit_price')::numeric, 0) <= 0 then
  ...
  values (public.next_invoice_number(), p_location_id, auth.uid(), subtotal, p_discount, total, p_idempotency_key, p_sale_type)
  returning * into sale;

  for item in select * from jsonb_array_elements(p_items) loop
    insert into public.transaction_items(transaction_id, product_id, quantity, unit_price)
    values (sale.id, (item->>'product_id')::uuid, (item->>'quantity')::integer, (item->>'unit_price')::numeric);
    ...
    values ((item->>'product_id')::uuid, p_location_id, 'SALE', -((item->>'quantity')::integer), 'TRANSACTION', sale.id, auth.uid());
  end loop;

  insert into public.payments(transaction_id, method, paid_amount, change_amount)
  values (sale.id, p_method, p_paid_amount, p_paid_amount - total);
  insert into public.audit_logs(user_id, role, location_id, action, reference_type, reference_id, description, metadata)
  values (auth.uid(), actor, p_location_id, 'SALE', 'TRANSACTION', sale.id, sale.invoice_no, jsonb_build_object('sale_type', p_sale_type));
  return sale;
end;
$$;
