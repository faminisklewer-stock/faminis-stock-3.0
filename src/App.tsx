    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="Main navigation">
          ...
          {visibleNavItems.map(({ label, icon: Icon, badge }: { label: string; icon: typeof LayoutDashboard; badge?: string }) => <button key={label} className={`nav-item ${active === label ? 'active' : ''}`} onClick={() => setActive(label)}><Icon size={18} /><span>{label}</span>{badge && <em>{badge}</em>}</button>)}
          {!isMasterUser && <><p className="nav-label nav-label-spaced">Operasional</p><div className="sync-card"><div className="sync-line"><span className="live-dot"></span><strong>Mode PWA</strong></div><span>{profile.role === 'OWNER' ? 'Laporan operasional' : 'Kasir, transfer, dan laporan'}</span></div></>}
          {isMasterUser && <><p className="nav-label nav-label-spaced">Pengaturan</p><button className={`nav-item ${active === 'Pelanggan' ? 'active' : ''}`} onClick={() => setActive('Pelanggan')}><Users size={18} /><span>Pelanggan</span></button><button className={`nav-item ${active === 'Akses tim' ? 'active' : ''}`} onClick={() => setActive('Akses tim')}><UserRound size={18} /><span>Akses tim</span></button></>}
        </nav>
        <div className="sidebar-bottom">{isMasterUser && <button className={`nav-item ${active === 'Pengaturan' ? 'active' : ''}`} onClick={() => setActive('Pengaturan')}><Settings size={18} /><span>Pengaturan</span></button>}<div className="sync-card"><div className="sync-line"><span className="live-dot"></span><strong>Sesi aman</strong></div><span>Terhubung ke Supabase</span></div><button className="profile-row" onClick={onLogout}><div className="avatar avatar-brown">{profile.full_name.slice(0, 2).toUpperCase()}</div><span><strong>{profile.full_name}</strong><small>{profile.role}</small></span></button></div>
      </aside>
...
function PosView({ profile, locations }: { profile: Profile; locations: Array<{ id: string; name: string }> }) {
  ...
  const availableLocations = canChooseLocation ? locations : locations.filter((location) => location.id === profile.location_id)
...
function TransfersView({ profile, locations }: { profile: Profile; locations: LocationOption[] }) {
  ...
  const transferStatusClass = (status: string) => {
    ...
    if (['REQUESTED', 'APPROVED', 'SHIPPED'].includes(status)) return 'transfer-badge in-progress'
    if (status === 'RECEIVED') return 'transfer-badge received'
    if (status === 'COMPLETED') return 'transfer-badge completed'
...
function ReportsView({ data, profile, onDownloadCsv, onDownloadPdf }: { data: DashboardData; profile: Profile; onDownloadCsv: () => void; onDownloadPdf: () => void }) {
  ...
  if (reportPeriod === 'daily') {
    ...
    periodStart.setDate(1)
    periodStart.setHours(0, 0, 0, 0)
  } else if (reportCustomFrom && reportCustomTo) {
...
function PurchasesView({ profile, locations }: { profile: Profile; locations: Array<{ id: string; name: string }> }) {
  ...
  const purchaseLocations = profile.role === 'MASTER' ? locations : locations.filter((location) => location.id === profile.location_id)
...
function CustomersView({ profile }: { profile: Profile }) {
  ...
  const [message, setMessage] = useState('')
  const [loading, setLoading] = useState(false)
  const [form, setForm] = useState({ id: '', full_name: '', segment: 'Retail', phone: '', email: '', address: '', status: 'Aktif', notes: '', active: true })

  const loadCustomers = useCallback(async () => {
    if (!client) return
    const { data, error: fetchError } = await client.from('customers').select('id, full_name, segment, phone, email, address, status, notes, active, created_at').order('full_name')
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setForm({ id: '', full_name: '', segment: 'Retail', phone: '', email: '', address: '', status: 'Aktif', notes: '', active: true })
    void loadCustomers()
  }
  ...
  return <section className="module-page"><div className="module-heading"><div><p className="eyebrow">CUSTOMERS</p><h1>Pelanggan</h1><p className="subtitle">Kelola daftar pelanggan dari Supabase.</p></div></div>{canManageCustomers ? <div className="panel"><div className="panel-heading"><div><h2>{form.id ? 'Edit pelanggan' : 'Tambah pelanggan baru'}</h2><p>Hanya MASTER atau WAREHOUSE yang dapat mengelola data pelanggan.</p></div></div><form className="customer-form" onSubmit={handleSubmit}><div className="two-col"><label>Nama lengkap<input value={form.full_name} onChange={(event) => setForm((current) => ({ ...current, full_name: event.target.value }))} placeholder="Nama pelanggan" /></label><label>Segmen<select value={form.segment} onChange={(event) => setForm((current) => ({ ...current, segment: event.target.value }))}><option value="Retail">Retail</option><option value="Reseller">Reseller</option><option value="B2B">B2B</option><option value="UMKM">UMKM</option></select></label></div><div className="two-col"><label>Telepon<input value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} placeholder="08xxxxxxxxxx" /></label><label>Status<select value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))}><option value="Aktif">Aktif</option><option value="Potensial">Potensial</option><option value="VIP">VIP</option><option value="Nonaktif">Nonaktif</option></select></label></div><div className="two-col"><label>Email<input type="email" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} placeholder="nama@email.com" /></label><label>Alamat<textarea rows={2} value={form.address} onChange={(event) => setForm((current) => ({ ...current, address: event.target.value }))} placeholder="Alamat lengkap pelanggan" /></label></div><label>Catatan<textarea rows={3} value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} placeholder="Catatan pelanggan, preferensi, atau histori transaksi" /></label><div className="toggle-row"><span>Status aktif</span><button type="button" className={`toggle-control ${form.active ? 'enabled' : 'disabled'}`} onClick={() => setForm((current) => ({ ...current, active: !current.active }))}><span className="toggle-thumb" /></button></div>{error && <p className="form-error">{error}</p>}{message && <p className="form-success">{message}</p>}<div className="inline-actions"><button className="button button-primary" type="submit" disabled={loading}>{loading ? 'Menyimpan...' : form.id ? 'Simpan perubahan' : 'Tambah pelanggan'}</button>{form.id && <button type="button" className="button button-secondary" onClick={() => setForm({ id: '', full_name: '', segment: 'Retail', phone: '', email: '', address: '', status: 'Aktif', notes: '', active: true })}>Batal</button>}</div></form></div> : <div className="panel"><div className="panel-heading"><div><h2>Mode baca</h2><p>Hanya MASTER atau WAREHOUSE yang dapat mengedit pelanggan.</p></div></div></div>}<div className="panel table-panel"><div className="panel-heading"><div><h2>Daftar pelanggan</h2><p>{customers.length} pelanggan terdaftar</p></div></div><div className="table-wrap"><table><thead><tr><th>Nama</th><th>Segmen</th><th>Alamat</th><th>Kontak</th><th>Status</th>{canManageCustomers && <th>Aksi</th>}</tr></thead><tbody>{customers.map((customer) => <tr key={customer.id}><td><strong>{customer.full_name}</strong>{customer.notes && <div className="muted-text">{customer.notes}</div>}</td><td>{customer.segment}</td><td>{customer.address ?? '-'}</td><td>{customer.phone ?? '-'}<br />{customer.email ?? '-'}</td><td><span className={`status ${customer.active ? 'positive' : 'negative'}`}><i></i>{customer.status || (customer.active ? 'Aktif' : 'Nonaktif')}</span></td>{canManageCustomers && <td><div className="inline-actions compact"><button type="button" className="text-button" onClick={() => setForm({ id: customer.id, full_name: customer.full_name, segment: customer.segment, phone: customer.phone ?? '', email: customer.email ?? '', address: customer.address ?? '', status: customer.status || 'Aktif', notes: customer.notes ?? '', active: customer.active })}>Edit</button><button type="button" className="text-button danger" onClick={() => void handleDelete(customer.id)}>Hapus</button></div></td>}</tr>)}</tbody></table></div></div></section>
}

function TeamAccessView({ profile, locations }: { profile: Profile; locations: LocationOption[] }) {
  const client = supabase
  ...
  function updateMemberRole(memberId: string, role: Profile['role']) {
    setMembers((current) => current.map((item) => {
      if (item.id !== memberId) return item
      const locationIsWarehouse = locations.some((location) => location.id === item.location_id && location.kind === 'WAREHOUSE')
      return { ...item, role, location_id: role === 'WAREHOUSE' && !locationIsWarehouse ? null : item.location_id }
    }))
  }

  return <section className="module-page"><div className="module-heading"><div><p className="eyebrow">TEAM ACCESS</p><h1>Akses tim</h1><p className="subtitle">Kelola role, lokasi, dan status pengguna dari Supabase.</p></div></div>{!canManageTeam ? <div className="panel"><div className="panel-heading"><div><h2>Mode baca</h2><p>Hanya MASTER yang dapat mengedit akses tim.</p></div></div></div> : null}{error && <p className="form-error">{error}</p>}{message && <p className="form-success">{message}</p>}<div className="panel table-panel"><div className="panel-heading"><div><h2>Daftar akses</h2><p>{members.length} anggota</p></div></div><div className="table-wrap"><table><thead><tr><th>Nama</th><th>Role</th><th>Lokasi</th><th>Status</th>{canManageTeam && <th>Aksi</th>}</tr></thead><tbody>{members.map((member) => <tr key={member.id}><td><strong>{member.full_name}</strong></td><td>{canManageTeam ? <select value={member.role} onChange={(event) => updateMemberRole(member.id, event.target.value as Profile['role'])}><option value="MASTER">MASTER</option><option value="OWNER">OWNER</option><option value="WAREHOUSE">WAREHOUSE</option><option value="LIVE">LIVE</option><option value="RUKO">RUKO</option></select> : member.role}</td><td>{canManageTeam ? <select value={member.location_id ?? ''} onChange={(event) => setMembers((current) => current.map((item) => item.id === member.id ? { ...item, location_id: event.target.value || null } : item))}><option value="">{member.role === 'WAREHOUSE' ? 'Pilih gudang' : 'Semua lokasi'}</option>{locations.filter((location) => member.role !== 'WAREHOUSE' || location.kind === 'WAREHOUSE').map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select> : (locations.find((location) => location.id === member.location_id)?.name ?? 'Semua lokasi')}</td><td>{canManageTeam ? <div className="toggle-row compact"><button type="button" className={`toggle-control ${member.active ? 'enabled' : 'disabled'}`} onClick={() => setMembers((current) => current.map((item) => item.id === member.id ? { ...item, active: !item.active } : item))}><span className="toggle-thumb" /></button><span>{member.active ? 'Aktif' : 'Nonaktif'}</span></div> : (member.active ? 'Aktif' : 'Nonaktif')}</td>{canManageTeam && <td><button type="button" className="button button-primary" onClick={() => void updateMember(member)}>Simpan</button></td>}</tr>)}</tbody></table></div></div></section>
}
