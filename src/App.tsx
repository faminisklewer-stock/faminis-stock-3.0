...
function ReportsView({ data, profile, onDownloadCsv, onDownloadPdf }: { data: DashboardData; profile: Profile; onDownloadCsv: () => void; onDownloadPdf: () => void }) {
  ...
  const isInReportPeriod = (createdAt: string) => !isOperationalUser || new Date(createdAt) >= periodStart && new Date(createdAt) <= now
  ...
  const reportModes: Array<{ key: 'penjualan' | 'transfer-masuk' | 'transfer-keluar' | 'stok'; label: string }> = isOperationalUser ? [
    ...
    { key: 'transfer-masuk', label: 'Transfer Masuk' },
  ...
  const renderReportPeriodFilter = () => isOperationalUser && <label className="report-period-filter"><span>Periode</span><select value={reportPeriod} onChange={(event) => setReportPeriod(event.target.value as typeof reportPeriod)}><option value="daily">Harian</option><option value="weekly">Mingguan</option><option value="monthly">Bulanan</option></select></label>
