...
export function MasterAuditLogsView() {
  ...
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  useEffect(() => { if (!supabase) { setLoading(false); return }; void supabase.from('audit_logs').select('id, action, description, location_id, created_at').order('created_at', { ascending: false }).limit(100).then(({ data, error: loadError }) => { if (loadError) setError('Audit log tidak dapat dimuat.'); else setLogs((data ?? []) as AuditRecord[]); setLoading(false) }) }, [])
  return <AdminPage eyebrow="SECURITY" title="Audit Log" subtitle="Jejak aktivitas operasional yang tercatat di database."><div className="panel table-panel"><div className="panel-heading"><div><h2>Aktivitas terbaru</h2><p>{logs.length} log ditampilkan</p></div><ScrollText size={20} /></div>{loading ? <div className="empty-state">Memuat audit log...</div> : error ? <div className="data-error">{error}</div> : <div className="table-wrap"><table><thead><tr><th>Waktu</th><th>Aksi</th><th>Deskripsi</th><th>Lokasi</th></tr></thead><tbody>{logs.map((log) => <tr key={log.id}><td>{new Date(log.created_at).toLocaleString('id-ID')}</td><td><strong>{log.action}</strong></td><td>{log.description ?? '-'}</td><td>{log.location_id ?? 'Global'}</td></tr>)}</tbody></table>{!logs.length && <div className="empty-state">Belum ada audit log.</div>}</div>}</div></AdminPage>
}
